...
Set `IFLOW_MCP_TRANSPORT=http` to serve MCP over SSE instead of stdio (default port from `PORT` or `3000`). **Remote mode defaults to `IFLOW_READ_ONLY=1`** until you set `IFLOW_READ_ONLY=0` explicitly. The process listens on **`IFLOW_HTTP_BIND_HOST`** (default **`127.0.0.1`**; use **`0.0.0.0`** in Docker). Responses include **`X-Request-Id`** (echoed from the incoming header or generated). **`GET /healthz`** (liveness) and **`GET /readyz`** (readiness) are unauthenticated. Readiness returns **503** if OAuth env is only partly set, or if `IFLOW_OAUTH_ISSUER`, `IFLOW_OAUTH_JWKS_URL`, and `IFLOW_MCP_AUDIENCE` are all set but the JWKS URL is unreachable or not a valid JWKS document. **`GET /sse`** and **`POST /messages`** require a Bearer token and JWT verification. Sample proxy and Compose files: [`ops/nginx.conf.sample`](ops/nginx.conf.sample), [`ops/compose.sample.yml`](ops/compose.sample.yml). See [`.plans/phase-b-remote-mcp.md`](.plans/phase-b-remote-mcp.md).
