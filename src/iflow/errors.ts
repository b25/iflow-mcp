      ...
      switch (error.body.code) {
        case "confirmation_required":
          return new McpError(
            ...
            `${error.body.message} Two-phase confirmation: run \`iflows-mcp confirm --key <IFLOW_API_POINTS key> [--token <confirm_token>] or set IFLOW_CONFIRM_TOKEN\` (same key as the tool call).`
          );
